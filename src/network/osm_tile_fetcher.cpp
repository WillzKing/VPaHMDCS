...
OsmTileFetcher::OsmTileFetcher(size_t threads) {
    for (size_t i = 0; i < threads; ++i) {
        workers.emplace_back(&OsmTileFetcher::WorkerLoop, this);
    }
}
...
void OsmTileFetcher::ProcessJob(const Job& job) {
    ...
    std::filesystem::path dir = std::filesystem::path("build") /
                                std::to_string(job.coord.zoom) /
                                std::to_string(job.coord.x);
    ...
    urlmaker << "https://tile.openstreetmap.org/"
             << job.coord.zoom << '/'
             << job.coord.x << '/'
             << job.coord.y << ".png";
    ...
    if (res == CURLE_OK && !blob.empty()) {
        ...
        if (blob.size() < MIN_VALID_TILE_SIZE) {
            LogError("[TILE_INVALID] Data too small: " +
                     std::to_string(blob.size()) + " bytes (min " +
                     std::to_string(MIN_VALID_TILE_SIZE) + ") for " + url);
            return;
        }
...
void OsmTileFetcher::Fetch(const std::vector<OsmTileCoord>& coords,
