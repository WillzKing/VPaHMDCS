...
void DrawMapWindow(bool& open, RuntimeState& st) {
    if (!open) return;
    if (!ImGui::Begin("Карта View", &open)) {
        ImGui::End();
        return;
    }
    ...
    if (ImPlot::BeginPlot("##osm_map", plotSize,
        ...
        if (ms.showHeat && ms.currentHeat.textureId && !ms.currentHeat.pixels.empty()) {
            ...
            if (isTileMode) {
                ...
                if (ImGui::Begin("Heatmap Info", nullptr,
                                 ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysAutoResize |
                                 ImGuiWindowFlags_NoMove)) {
                    ImGui::TextColored(ImVec4(1, 1, 0, 1),
                                       "Режим по тайлам: генерация на тайл пока не реализована.");
                    ImGui::Text("Global map: %dx%d px", ms.currentHeat.width, ms.currentHeat.height);
                    ImGui::End();
                }
            } else {
